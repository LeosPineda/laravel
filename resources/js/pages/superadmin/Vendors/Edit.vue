...
const logoPreview = ref<string | null>(props.vendor.brand_logo ? `/storage/${props.vendor.brand_logo}` : null);

const form = useForm({
    name: props.vendor.user.name,
    password: '',
    brand_name: props.vendor.brand_name,
...
const removeLogo = () => {
    ...
    logoPreview.value = props.vendor.brand_logo ? `/storage/${props.vendor.brand_logo}` : null;
};

const submit = () => {
    form.post(`/superadmin/vendors/${props.vendor.id}`, {
        forceFormData: true,
    });
};
...
<template>
    ...
    <div class="min-h-screen bg-[#F5F5F5]">
                ...
                <div class="flex justify-between items-center h-16">
                    <div class="flex items-center gap-8">
                        ...
                        <nav class="hidden md:flex items-center gap-1">
                            <Link href="/superadmin/dashboard" class="px-4 py-2 text-sm font-medium text-[#1A1A1A]/70 hover:text-[#1A1A1A] hover:bg-[#F5F5F5] rounded-lg transition-colors">
                                Dashboard
                            </Link>
                            <Link href="/superadmin/vendors" class="px-4 py-2 text-sm font-medium text-white bg-[#FF6B35] rounded-lg">
                                Vendors
                            </Link>
                        </nav>
                    </div>
        ...
        <main class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <!-- Back Link -->
            <Link href="/superadmin/vendors" class="inline-flex items-center gap-2 text-sm font-medium text-[#1A1A1A]/60 hover:text-[#FF6B35] transition-colors mb-6">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Page Header -->
            <div class="mb-8">
                <h1 class="text-2xl font-bold text-[#1A1A1A]">Edit Vendor</h1>
                <p class="text-[#1A1A1A]/60 mt-1">Update vendor account and branding</p>
            </div>
                ...
                <form @submit.prevent="submit">
                    ...
                    <div class="p-6 border-b border-[#E0E0E0]">
                        ...
                        <div class="space-y-4">
                            <!-- Brand Name -->
                            <div>
                                <label for="brand_name" class="block text-sm font-semibold text-[#1A1A1A] mb-2">Brand Name</label>
                                <input id="brand_name" type="text" v-model="form.brand_name" required placeholder="e.g., Jollibee, McDonald's"
                                    class="w-full px-4 py-3.5 border border-[#E0E0E0] rounded-xl bg-[#F5F5F5]/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#FF6B35]/20 focus:border-[#FF6B35] transition-all text-[#1A1A1A]"
                            ...
                            <!-- Brand Logo -->
                            <div>
                                <label class="block text-sm font-semibold text-[#1A1A1A] mb-2">Brand Logo</label>
                                    ...
                                    <div class="flex-1">
                                        <label class="block w-full cursor-pointer">
                                            <div class="px-4 py-3 border border-[#E0E0E0] rounded-xl bg-[#F5F5F5]/50 hover:bg-[#F5F5F5] transition-colors text-center">
                                                <span class="text-sm font-medium text-[#1A1A1A]">Choose new logo</span>
                                        ...
                                        <button v-if="form.brand_logo" type="button" @click="removeLogo" class="mt-2 text-sm text-red-600 hover:underline">
                                            Cancel new logo
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-[#1A1A1A]/60">Leave password empty to keep current</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <!-- Owner Name -->
                            <div>
                                <label for="name" class="block text-sm font-semibold text-[#1A1A1A] mb-2">Owner Name</label>
