...
import { ArrowLeft, Plus, X, Upload } from 'lucide-vue-next';

const form = useForm({
    name: '', price: '', category: '', stock_quantity: 10, image: null as File | null,
...
const addAddon = () => form.addons.push({ name: '', price: '' });
const removeAddon = (i: number) => form.addons.splice(i, 1);

const submit = () => {
    form.post('/vendor/products', { forceFormData: true });
};
</script>
        ...
        <div class="max-w-2xl">
            ...
            <form @submit.prevent="submit" class="bg-white rounded-xl border border-[#E0E0E0] p-6 space-y-6">
                ...
                <div>
                    ...
                    <input v-model="form.category" type="text" placeholder="e.g., Chicken, Rice, Drinks" class="w-full px-4 py-2 border border-[#E0E0E0] rounded-lg" />
                </div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <button type="button" @click="addAddon" class="text-[#FF6B35] text-sm flex items-center gap-1"><Plus class="w-4 h-4" /> Add</button>
                    </div>
                    <div v-for="(addon, i) in form.addons" :key="i" class="flex gap-2 mb-2">
                        <input v-model="addon.name" type="text" placeholder="Addon name" class="flex-1 px-3 py-2 border border-[#E0E0E0] rounded-lg" />
                        <input v-model="addon.price" type="number" step="0.01" min="0" placeholder="Price" class="w-24 px-3 py-2 border border-[#E0E0E0] rounded-lg" />
                        <button type="button" @click="removeAddon(i)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg"><X class="w-5 h-5" /></button>
                    </div>
                </div>
